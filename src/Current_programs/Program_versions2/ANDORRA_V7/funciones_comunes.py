...
def navegar_calle_por_rumbo():
    ...
    error moderado -> lento; error GRANDE (mas de UMBRAL_ERROR_GRANDE_RUMBO,
    tipico justo tras cortar una esquina sin haber girado el chasis todavia)
    -> muy lento, casi un giro sobre el sitio, porque un giro tan grande sin
    apenas avanzar necesita mucho menos espacio real que intentarlo a la vez
    que se avanza con normalidad.

    Correccion de DERIVA hacia una pared: si rumbo_calle[] no es exactamente
    paralelo al pasillo real (pequeño error de medicion o imperfeccion de la
    pista), el error de rumbo puede quedarse dentro de la zona muerta todo
    el rato sin que la brujula "se entere" de que esta derivando poco a poco
    hacia una pared. En vez de reaccionar solo a la distancia puntual (que
    no distingue "pasillo estrecho de por si" de "me estoy acercando de
    verdad"), se compara cada lectura en los extremos +-90 del barrido con
    la ULTIMA lectura de ese mismo lado: si ha decrecido y esta por debajo
    de DISTANCIA_MINIMA_PARED_CM, se acumula un pequeño sesgo (con tope) que
    se suma al rumbo objetivo, alejando progresivamente al robot de esa
    pared sin necesitar ningun "modo pared" que contradiga al rumbo - las
    dos correcciones conviven en la misma formula. El sesgo se reinicia al
    cambiar de calle.
    ...
    sesgo_pared = 0.0
    ultima_dist_izq = None
    ultima_dist_der = None
    ...
    while True:
        ...
        elif abs(error) > hardware.UMBRAL_ERROR_GRANDE_RUMBO:
            hardware.servoTraccion.duty_u16(hardware.Avanza_giro_cerrado)
        else:
        ...
        # Deriva hacia la pared izquierda: solo se comprueba justo en el
        # extremo -90 del barrido, comparando con la ultima vez que se paso
        # por ese mismo extremo.
        if angulo_oscilacion <= -hardware.ANGULO_MAX_BARRIDO_CRUCERO and distancia_tf is not None:
            if ultima_dist_izq is not None and distancia_tf < ultima_dist_izq \
                    and distancia_tf < hardware.DISTANCIA_MINIMA_PARED_CM:
                sesgo_pared = min(sesgo_pared + hardware.SESGO_PARED_INCREMENTO_DEG, hardware.SESGO_PARED_MAX_DEG)
                print(f"NAVEGAR_CALLE: acercandose a pared IZQUIERDA ({distancia_tf}cm < {ultima_dist_izq}cm) "
                      f"-> sesgo={sesgo_pared:.1f}")
            ultima_dist_izq = distancia_tf

        # Deriva hacia la pared derecha: mismo mecanismo, extremo +90.
        elif angulo_oscilacion >= hardware.ANGULO_MAX_BARRIDO_CRUCERO and distancia_tf is not None:
            if ultima_dist_der is not None and distancia_tf < ultima_dist_der \
                    and distancia_tf < hardware.DISTANCIA_MINIMA_PARED_CM:
                sesgo_pared = max(sesgo_pared - hardware.SESGO_PARED_INCREMENTO_DEG, -hardware.SESGO_PARED_MAX_DEG)
                print(f"NAVEGAR_CALLE: acercandose a pared DERECHA ({distancia_tf}cm < {ultima_dist_der}cm) "
                      f"-> sesgo={sesgo_pared:.1f}")
            ultima_dist_der = distancia_tf
        ...
        print(f"NAVEGAR_CALLE: calle={hardware.calle_actual} rumbo_obj={rumbo_objetivo:.1f} "
              ...
              f"tf={distancia_tf} sesgo_pared={sesgo_pared:.1f}")
        ...
        # Calle nueva (esquina cruzada): el sesgo era especifico de la calle
        # anterior, no tiene sentido arrastrarlo a la siguiente.
