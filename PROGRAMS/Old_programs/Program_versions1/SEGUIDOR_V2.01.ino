...
huskylens.setCustomName("rojo", rojo);
huskylens.setCustomName("verde", verde);
delay(100);
...
//scan1();
  ...
  if (huskylens.request() && huskylens.available()) {
    ...
    if (color.command == COMMAND_RETURN_BLOCK) {
      if (color.ID == rojo) {
        // Cálculo del ángulo de dirección centrado en 160
        dir = 90 + 0.5 * (160 - color.xCenter);
        dir = constrain(dir, 50, 130); 
        
        servoDireccion.write(dir); 
        servoTraccion.write(70);
        distFR=hc.dist(1);
        if ((distFR<15)&&(distFR>0)){
        servoTraccion.write(Para);
        delay(5000);
        }
      } 
