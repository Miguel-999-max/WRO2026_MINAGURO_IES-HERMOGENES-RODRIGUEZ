...
* **05/03/26:** We started looking for components in our classroom that could be used to build the robot because we must use the free stuff the Councilor of Education of CLM has given to every school to ensure a level playing field. We also researched strategies from previous years on GitHub. 
...
* **25/08/26:** By placing the lead weight at the front, we have managed to stop the robot’s front wheels from skidding. However, we have noticed that we are still experiencing the same problems we had some time ago. Firstly, when approaching a block, it continues to reverse, even when it is not necessary. Secondly, when passing a block, sometimes it does so correctly and other times it carries on forwards, knocking the obstacle over. We believe this is because, when it is about to avoid an object, the TFmini-S servo and the Husky Lens should instantly move to 90° on the corresponding side once it has passed the obstacle, but it does not do so. To solve both problems, we are going to make it turn right first, move straight ahead for a short while, turn left and then move straight ahead again. The third problem we have observed is that when it aligns with a pillar, the direction straightens out gradually, and this costs us quite a bit of time. That is why we have decided it should continue to do so, but at a slightly faster pace so as not to lose so much time. Finally, we’ve carried out the necessary calculations to determine: the torque, the revolutions per minute and the maximum speed the robot reaches.

* **26/08/26:** Today we carried out a test with three red blocks in a row. At the second obstacle, as soon as it had finished going round it and just before checking whether it had passed it, the robot performed a sweep to check for further blocks, rather than doing so without moving the servo. As a result, the robot detected this second obstacle again, avoided it once more, and has now moved on to the third block. The solution to this problem is that, instead of simply using the Husky Lens to check for an obstacle, we’ll use the TFmini-S to determine whether it’s more than 30 cm away, to ensure it isn’t avoiding the same obstacle again. Furthermore, we’ve noticed that sometimes, during the initial scan, it detects the car park wall and mistakes it for a block. So we’ve added a small straight-line movement to prevent this confusion.

* **27/08/26:** We started by focusing on the problem of the robot getting too close to the wall. What we’ve done is make it check the distance to the wall on the left and on the right; if either is less than 30 cm, it means it’s close to a wall and must therefore correct its course by adding a value to the compass heading. However, we’ve found that when implementing yesterday’s modification, we’ve uncovered a new problem. After finishing going round the obstacle, if it spots an object before checking whether it has already passed it or not, it heads straight for it. The problem is that sometimes it spots an obstacle in the distance and ignores the one that is closer. Therefore, we are going to programme it so that, upon finishing going round the block, it checks whether there is an object in front of it, but by scanning 30° to each side. After sorting out this problem, we’ve spotted another one. We’ve now realised that it no longer backs up if it’s very close to a block in order to avoid it more effectively. Instead, it ploughs straight through it and ends up facing the same object again. We’ve now programmed it to back up when the object is at a distance of 25 cm or less. But we’ll leave that to sort out tomorrow.

* **28/08/26:** Today we have once again focused on improving our documentation.

* **31/08/26:** Let’s pick up where we left off the other day. We’ve now identified why the robot was previously knocking the block over: it’s because we’ve programmed it to start dodging when the object is 15 cm away, but what actually happens is that it starts dodging when it’s 5 cm away, which is why it ends up knocking it over. This is because, immediately after taking the reading, the camera has to be repositioned, the LED switched on and other tasks carried out within the same loop; consequently, until it has completed all of these, it does not detect that it has reached the block or start to swerve. As a solution, we’ve programmed it to check whether it has reached the block immediately after sending the reading, and only then carry out everything else. And now it only reverses in extreme cases where it is 7 cm or less away.

* **01/09/26:** We managed to get our robot to complete three full laps of the circuit in the obstacle challenge, starting from the parking area, but we were unable to return and park in the parking area, and we had to remove the parking area before completing the first lap as we spotted a problem. Due to the quality of the Husky Lens, it recognises the parking area as a red block. We have therefore decided to change our strategy. We will now start the obstacle challenge without the parking area.

* **02/09/26:** Hemos empezado a lanzar ideas para ver cómo resolvemos la decisión inicial, la que hace que sepa si está en reto libre o con obstáculos y si está girando a sentido horario o antihorario. Hemos llegado a la conclusión de que el robot debería ir marcha atrás y, tras ello, que la Husky Lens mire hacia la izquierda y hacia la derecha para ver si hay bloques. Si los encuentra, para saber hacia qué sentido está, miraríamos si la coordenada X del objeto es mayor de 160, giraría en sentido horario, y si por lo contrario es menor de 160, giraría en sentido antihorario. Si aun así no los ve, tiene que ir hacia delante hasta sobrepasar la esquina y hacer la primera curva. Si no ve ningún bloque significa que está en el reto libre y, si por lo contrario ve alguno, está en el reto con obstáculos.

* **03/09/26:** Today we’ve come up with an idea regarding the robot’s speed, as the motor isn’t running as fast as we’d like. The idea is to fit a larger gear than the one we currently have. We’d fit a 50-tooth gear so that the robot can complete the challenges more quickly. We’ve also decided to remove the edges from our base so that it doesn’t knock over any blocks or hit the walls, although we’re not sure when we’ll implement this change. Also, whilst running the programme for the obstacle challenge, we decided to remove the feature where it checks for walls every now and then to avoid collisions, as it sometimes gets confused by this. Finally, we’ve noticed that in the latest tests our robot confuses the orange lines with red blocks, but we’ll look into this further tomorrow.

* **04/09/26:** As we mentioned yesterday, our camera was mistaking the orange lines on the circuit for red blocks and ended up crashing into the wall. Well, as soon as we arrived today, we tried to track down the error and discovered that it is due to the lighting in the classroom where we are working; when we focused on the areas where it was getting confused, using the torches on our phones, it worked perfectly. However, we are still going to try to fix the problem by making smaller sweeps around the bends so that it does not detect the orange line as a red block.
...
| :---: | :---: | :---: | :---: | :---: |
| SPT5632-360 | 1 | <img src="other/components/SPT5632-360.jpg" width="150" height="120"> | Robot mobility | <a href="https://de.banggood.com/SPT-Servo-SPT5632-360-Coreless-Digital-Servo-Metal-Gear-Lager-Torque-Linear-Change-For-RC-Robot-p-1418818.html?akmClientCountry=ES&cur_warehouse=CN"> 🛒 Shop</a> |
| MS18 | 1 | <img src="other/components/MS18.jpg" width="150" height="120"> | Robot direction system | <a href="https://es.aliexpress.com/item/1005012158832496.html?spm=a2g0o.productlist.main.4.1edfnKTwnKTwnB&algo_pvid=5417366f-c791-4e53-a569-c27a98fc2162&algo_exp_id=5417366f-c791-4e53-a569-c27a98fc2162-3&pdp_ext_f=%7B%22order%22%3A%228%22%2C%22spu_best_type%22%3A%22price%22%2C%22eval%22%3A%221%22%2C%22fromPage%22%3A%22search%22%7D&pdp_npi=6%40dis%21EUR%216.85%212.65%21%21%2153.52%2120.74%21%40210384b217781457573274739eee2a%2112000057659670774%21sea%21ES%210%21ABX%211%210%21n_tag%3A-29910%3Bd%3Aa4bac484%3Bm03_new_user%3A-29895%3BpisId%3A5000000205922472&curPageLogUid=1s0ozgZzGPqi&utparam-url=scene%3Asearch%7Cquery_from%3A%7Cx_object_id%3A1005012158832496%7C_p_origin_prod%3A"> 🛒 Shop</a> |
...
| Raspberry pi pico 2 rp2350 | 1 | <img src="other/components/SC21034-40.jpg" width="150" height="120"> | Robot controller | <a href="https://www.raspberrypi.com/products/raspberry-pi-pico-2/"> 🛒 Shop</a> |
...
| Electrolytic condenser 0,1 microfarad | 1 | <img src="other/components/Electrolytic condenser 0,1 microfarad.webp" width="150" height="120"> | (maker recommendation) | <a href="https://www.ebay.com/itm/202550513396"> 🛒 Shop</a> |
...
| Micro Servo MG90S | 1 | <img src="other/components/Micro Servo MG90S.jpg" width="150" height="120"> | Allow the camera to move and have a wider field of view | <a href="https://www.hta3d.com/es/microservo-mg90s-180-de-posicionamiento-engranajes-metalicos"> 🛒 Shop</a> |"> Camera servo link </a> |
...
| Counterweight | 2 | <img src="other/components/metal.jpg" width="150" height="120"> | We use it to adjust and balance the robot’s weight, as the rear is quite heavy due to the spoiler housing the batteries, which is why we place it at the front. In this case, there are two of them, and each one weighs 40 grams | <a href="https://selectauto.es/consumibles-para-ruedas/6175-contrapesos-para-llantas-de-aleacion-40gx50-uds-marca-seltech-5949128213693.html"> 🛒 Shop</a> |
...
| Adapter wheels in 3D | 2 | <img src="models/Wheel adapter.png" width="150" height="120"> | Connect the wheels to the differential | [`📥Download`](models/wheel%20adapter.stl) |
...
| Battery support 2 | 1 | <img src="models/soportebateria2.jpg" width="150" height="120"> | Support batteries | [`📥Download`](models/soportebateria2.stl) |
| Switch support | 1 | <img src="models/SOPORTE_INTERRUPTOR.jpg" width="150" height="120"> | Support the switch | [`📥Download`](models/SOPORTE_INTERRUPTOR.stl) |
| Adapter wheels in 3D | 2 | <img src="models/Wheel adapter.png" width="150" height="120"> | Supply energy to huskylens directly of the batteries | [`📥Download`](models/wheel%20adapter.stl) |
| Adapter gear in 3D | 1 | <img src="models/Gear adapter.png" width="150" height="120"> | Gearing with 360º servo gear | [`📥Download`](models/GEAR_ADAPTER.stl) |
| Magnetometer mast | 1 | <img src="models/soporte_magnetometro.png" width="150" height="120"> | Support the magnetometer | [`📥Download`](models/SOPORTE_10-DOF.stl) |
</details>
